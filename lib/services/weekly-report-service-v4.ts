...
export class WeeklyReportServiceV4 {
  private static resend = new Resend(process.env.RESEND_API_KEY)

  static async sendWeeklyReport(techCompanyId: string) {
    console.log(`[WeeklyReportServiceV4] === INICIO sendWeeklyReport para ${techCompanyId} ===`)

    try {
      const supabase = createServerClient()

      // 1. Obtener tech company
      console.log(`[WeeklyReportServiceV4] 1. Obteniendo tech company...`)
      ...
      if (techError || !techCompany) {
        console.error(`[WeeklyReportServiceV4] Error tech company:`, techError)
        return { success: false, results: [], error: "Tech company not found" }
      }
      console.log(`[WeeklyReportServiceV4] Tech company OK:`, techCompany.name)

      // 2. Obtener destinatarios
      console.log(`[WeeklyReportServiceV4] 2. Obteniendo destinatarios...`)
      ...
      if (recipientError) {
        console.error(`[WeeklyReportServiceV4] Error recipients:`, recipientError)
        return { success: false, results: [], error: "Error getting recipients" }
      }

      if (!recipientData || recipientData.length === 0) {
        console.log(`[WeeklyReportServiceV4] No recipients found`)
        return { success: false, results: [], error: "No recipients configured" }
      }

      const userIds = recipientData.map((r) => r.user_id)
      console.log(`[WeeklyReportServiceV4] Found ${userIds.length} recipient IDs`)

      // 3. Obtener datos de usuarios
      console.log(`[WeeklyReportServiceV4] 3. Obteniendo datos de usuarios...`)
      ...
      if (userError) {
        console.error(`[WeeklyReportServiceV4] Error users:`, userError)
        return { success: false, results: [], error: "Error getting users" }
      }

      if (!userData || userData.length === 0) {
        console.log(`[WeeklyReportServiceV4] No users found`)
        return { success: false, results: [], error: "No users found" }
      }

      console.log(
        `[WeeklyReportServiceV4] Found ${userData.length} users:`,
        userData.map((u) => u.email),
      )

      // 4. Verificar configuración de email
      if (!process.env.RESEND_API_KEY) {
        console.error(`[WeeklyReportServiceV4] RESEND_API_KEY not configured`)
        return { success: false, results: [], error: "RESEND_API_KEY not configured" }
      }

      // 5. Obtener oportunidades
      console.log(`[WeeklyReportServiceV4] 4. Obteniendo oportunidades...`)
      ...
      if (oppError) {
        console.error(`[WeeklyReportServiceV4] Error opportunities:`, oppError)
        return { success: false, results: [], error: `Error getting opportunities: ${oppError.message}` }
      }

      const allOpportunities = opportunities || []
      console.log(`[WeeklyReportServiceV4] Found ${allOpportunities.length} opportunities`)
      ...
      // 11. Enviar emails
      console.log(`[WeeklyReportServiceV4] 5. Enviando emails...`)
      const results = []
      const emailFrom = process.env.NEXT_PUBLIC_EMAIL_FROM || "ScaleUp CRM <no-reply@scaleup-global.com>"

      for (const user of userData) {
        try {
          console.log(`[WeeklyReportServiceV4] Enviando a ${user.email}`)
          ...
          if (error) {
            console.error(`[WeeklyReportServiceV4] Error enviando a ${user.email}:`, error)
            results.push({
              email: user.email,
              success: false,
            ...
            console.log(`[WeeklyReportServiceV4] Email enviado exitosamente a ${user.email}`)
            results.push({
              email: user.email,
              success: true,
          ...
          console.error(`[WeeklyReportServiceV4] Error inesperado enviando a ${user.email}:`, error)
          results.push({
            email: user.email,
            success: false,
      ...
      console.log(`[WeeklyReportServiceV4] Completado: ${successCount}/${results.length} emails enviados`)
      ...
      console.error(`[WeeklyReportServiceV4] Error general:`, error)
      return {
        success: false,
        results: [],
  ...
  private static generateEnhancedHtml(reportData: any): string {
    const { techCompany, summary, opportunitiesByPartner, generatedAt } = reportData
    ...
    const generateImprovedChart = () => {
      const data = [
        { label: "Activas", value: summary.totalActiveOpportunities, color: scaleupBlue },
        { label: "Nuevas", value: summary.newThisWeek, color: "#28a745" },
        { label: "Con Actividad", value: summary.withActivity, color: "#17a2b8" },
        { label: "Ganadas", value: summary.wonThisWeek, color: "#ffc107" },
      ]
      ...
      const bars = data
        .map((item, index) => {
          const barHeight = Math.max((item.value / maxValue) * 120, 5) // Mínimo 5px de altura
          const barWidth = 80
          ...
          return `
          <!-- Barra -->
          <rect x="${x}" y="${y}" width="${barWidth}" height="${barHeight}" fill="${item.color}" rx="4" stroke="#fff" stroke-width="2"/>
          
          <!-- Valor encima de la barra -->
          <text x="${x + barWidth / 2}" y="${y - 8}" text-anchor="middle" font-size="14" font-weight="bold" fill="#333">${item.value}</text>
          
          <!-- Etiqueta debajo -->
          <text x="${x + barWidth / 2}" y="175" text-anchor="middle" font-size="12" fill="#666">${item.label}</text>
          
          <!-- Línea de separación -->
          ${index < data.length - 1 ? `<line x1="${x + barWidth + 10}" y1="30" x2="${x + barWidth + 10}" y2="150" stroke="#e0e0e0" stroke-width="1"/>` : ""}
        `
        })
        .join("")

      return `
        <div style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); padding: 20px; border-radius: 12px; text-align: center; margin: 20px 0; border: 1px solid #dee2e6;">
          <h3 style="color: ${scaleupBlue}; margin-bottom: 15px; font-size: 16px;">📊 Resumen Visual</h3>
          <svg width="${chartWidth}" height="${chartHeight}" style="margin: 0 auto; display: block;">
            <!-- Fondo del gráfico -->
            <rect x="20" y="20" width="${chartWidth - 40}" height="${chartHeight - 60}" fill="#ffffff" rx="8" stroke="#e0e0e0" stroke-width="1"/>
            
            <!-- Líneas de referencia -->
            <line x1="40" y1="150" x2="${chartWidth - 20}" y2="150" stroke="#e0e0e0" stroke-width="1"/>
    ...
    return `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px; background-color: #f8f9fa;">
        <!-- Header -->
        <div style="background: linear-gradient(135deg, ${scaleupBlue} 0%, #003d82 100%); color: white; padding: 25px; border-radius: 12px; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
