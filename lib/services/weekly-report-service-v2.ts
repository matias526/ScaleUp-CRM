...
export class WeeklyReportServiceV2 {
  private static resend = new Resend(process.env.RESEND_API_KEY)

  static async sendWeeklyReport(techCompanyId: string) {
    console.log(`[WeeklyReportServiceV2] === INICIO sendWeeklyReport para ${techCompanyId} ===`)

    try {
      const supabase = createServerClient()

      // 1. Obtener tech company
      console.log(`[WeeklyReportServiceV2] 1. Obteniendo tech company...`)
      ...
      if (techError || !techCompany) {
        console.error(`[WeeklyReportServiceV2] Error tech company:`, techError)
        return { success: false, results: [], error: "Tech company not found" }
      }
      console.log(`[WeeklyReportServiceV2] Tech company OK:`, techCompany.name)

      // 2. Obtener destinatarios
      console.log(`[WeeklyReportServiceV2] 2. Obteniendo destinatarios...`)
      ...
      if (recipientError) {
        console.error(`[WeeklyReportServiceV2] Error recipients:`, recipientError)
        return { success: false, results: [], error: "Error getting recipients" }
      }

      if (!recipientData || recipientData.length === 0) {
        console.log(`[WeeklyReportServiceV2] No recipients found`)
        return { success: false, results: [], error: "No recipients configured" }
      }

      const userIds = recipientData.map((r) => r.user_id)
      console.log(`[WeeklyReportServiceV2] Found ${userIds.length} recipient IDs`)

      // 3. Obtener datos de usuarios
      console.log(`[WeeklyReportServiceV2] 3. Obteniendo datos de usuarios...`)
      ...
      if (userError) {
        console.error(`[WeeklyReportServiceV2] Error users:`, userError)
        return { success: false, results: [], error: "Error getting users" }
      }

      if (!userData || userData.length === 0) {
        console.log(`[WeeklyReportServiceV2] No users found`)
        return { success: false, results: [], error: "No users found" }
      }

      console.log(
        `[WeeklyReportServiceV2] Found ${userData.length} users:`,
        userData.map((u) => u.email),
      )

      // 4. Verificar configuración de email
      if (!process.env.RESEND_API_KEY) {
        console.error(`[WeeklyReportServiceV2] RESEND_API_KEY not configured`)
        return { success: false, results: [], error: "RESEND_API_KEY not configured" }
      }

      // 5. Obtener oportunidades - CORREGIDO: solo usar pipeline_stages.code
      console.log(`[WeeklyReportServiceV2] 4. Obteniendo oportunidades...`)

      const oneWeekAgo = subDays(new Date(), 7)

      // Consulta corregida - solo usar code, no name
      ...
      if (oppError) {
        console.error(`[WeeklyReportServiceV2] Error opportunities:`, oppError)
        return { success: false, results: [], error: `Error getting opportunities: ${oppError.message}` }
      }

      const allOpportunities = opportunities || []
      console.log(`[WeeklyReportServiceV2] Found ${allOpportunities.length} opportunities`)

      // 6. Obtener actividad de la semana (simplificado)
      ...
      // 7. Clasificar oportunidades - CORREGIDO: usar solo code
      ...
      const closedOpportunities = allOpportunities.filter((opp) =>
        ["won", "lost"].includes(opp.pipeline_stages?.code?.toLowerCase() || ""),
      )
      const openOpportunities = allOpportunities.filter(
        (opp) => !["won", "lost"].includes(opp.pipeline_stages?.code?.toLowerCase() || ""),
      )
      ...
      const reportData = {
        ...
        summary: {
          ...
          closedThisWeek: closedOpportunities.filter((opp) => opp.updated_at && new Date(opp.updated_at) >= oneWeekAgo)
            .length,
          withActivity,
          withoutActivity,
        },
        opportunities: {
          ...
          closed: closedOpportunities.slice(0, 10),
        },
      ...
      // 10. Enviar emails
      console.log(`[WeeklyReportServiceV2] 5. Enviando emails...`)
      const results = []
      const emailFrom = process.env.NEXT_PUBLIC_EMAIL_FROM || "ScaleUp CRM <no-reply@scaleup-global.com>"

      for (const user of userData) {
        try {
          console.log(`[WeeklyReportServiceV2] Enviando a ${user.email}`)
          ...
          if (error) {
            console.error(`[WeeklyReportServiceV2] Error enviando a ${user.email}:`, error)
            results.push({
              email: user.email,
              success: false,
            ...
            console.log(`[WeeklyReportServiceV2] Email enviado exitosamente a ${user.email}`)
            results.push({
              email: user.email,
              success: true,
          ...
          console.error(`[WeeklyReportServiceV2] Error inesperado enviando a ${user.email}:`, error)
          results.push({
            email: user.email,
            success: false,
      ...
      console.log(`[WeeklyReportServiceV2] Completado: ${successCount}/${results.length} emails enviados`)
      ...
      console.error(`[WeeklyReportServiceV2] Error general:`, error)
      return {
        success: false,
        results: [],
  ...
  private static generateEnhancedHtml(reportData: any): string {
    ...
    // Función para traducir códigos de etapa a nombres legibles
    const getStageDisplayName = (code?: string) => {
      ...
      return stageNames[code.toLowerCase()] || code
    }
    ...
    return `
      <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background-color: white; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px; margin-top: 20px;">
          ...
          ${generateOpportunityTable(opportunities.closed, "✅ Oportunidades Cerradas (esta semana)", "#fff3e0")}
