...
export default function PartnerTasks({ partnerId, partnerName }: PartnerTasksProps) {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        setIsLoading(true)
        const data = await taskService.getTasksByPartnerId(partnerId)
        setTasks(data)
      } catch (err) {
        console.error("Error fetching partner tasks:", err)
        setError("Error al cargar las tareas del partner")
      } finally {
        setIsLoading(false)
      }
    }
  ...
  return (
    <Card className="mt-6">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="text-xl font-bold">{t("partner.tasks.title", "Tareas del Partner")}</CardTitle>
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="text-center py-4">
            {t("partner.tasks.no_tasks", "No hay tareas asociadas a este partner")}
          </div>
        ) : (
              ...
              <TableBody>
                {tasks.map((task) => (
                  <TableRow key={task.id}>
                    ...
                    <TableCell>
                      <Badge className={getStatusColor(task.status)}>{getStatusTranslation(task.status)}</Badge>
                    </TableCell>
                    <TableCell>
