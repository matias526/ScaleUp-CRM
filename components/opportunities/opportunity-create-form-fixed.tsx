...
export function OpportunityCreateFormFixed() {
  ...
  // SOLUCIÓN: Usar useRef para mantener los datos persistentes
  const persistentData = useRef<OpportunityFormData>({
    title: "",
    description: "",
  ...
  // Estado para forzar re-render cuando cambian los datos persistentes
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: persistentData.current,
    mode: "onChange",
  })

  // Función para actualizar datos persistentes
  const updatePersistentData = (field: keyof OpportunityFormData, value: any) => {
    console.log(`🔄 Actualizando ${field}:`, value)
    persistentData.current[field] = value
    form.setValue(field, value)
    triggerUpdate()
  }

  // Función para obtener datos actuales
  const getCurrentData = (): OpportunityFormData => {
    return { ...persistentData.current }
  }
      ...
      try {
        ...
        if (!isScaleUpUser) {
          ...
          if (leadStage) {
            ...
            updatePersistentData("pipeline_stage_id", leadStage.id)
          }
        } else if (stagesData.length > 0 && !preselectedStageId) {
          console.log("🎯 Asignando primera etapa:", stagesData[0].id)
          updatePersistentData("pipeline_stage_id", stagesData[0].id)
        }
  ...
  const loadPartnersForTechCompany = async (techCompanyId: string) => {
    if (!techCompanyId) {
  ...
  const loadCountriesForPartner = async (partnerId: string | null) => {
    ...
    try {
      ...
      if (countriesData.length === 1) {
        ...
        updatePersistentData("country", countriesData[0].code)
      }
  ...
  const loadUsersForPartner = async (partnerId: string | null) => {
    if (!partnerId || partnerId === NO_PARTNER_VALUE) {
      setPartnerUsers([])
      updatePersistentData("partner_responsible_id", null)
      return
    }
    ...
    try {
      ...
      if (users.length > 0) {
        ...
        updatePersistentData("partner_responsible_id", users[0].id)
      }
  ...
  // Cargar tech fields
  const loadTechFields = async (techCompanyId: string) => {
    if (!techCompanyId) {
      setTechFields([])
      setTechFieldValues({})
    ...
    try {
      ...
      updatePersistentData("tech_field_ids", fieldIds)
  ...
  const handleTechCompanyChange = (value: string) => {
    console.log("🏢 Tech company cambiado a:", value)
    updatePersistentData("tech_company_id", value)
    updatePersistentData("partner_id", null)
  ...
  const handlePartnerChange = (value: string | null) => {
    ...
    updatePersistentData("partner_id", value)
    updatePersistentData("country", "")

    loadCountriesForPartner(value)
    loadUsersForPartner(value)
  }

  const handleEndCustomerChange = (value: string | null) => {
    console.log("👥 End customer cambiado a:", value)
    updatePersistentData("end_customer_id", value)
  }

  // Handle form submission
  async function onSubmit(values: FormValues) {
    console.log("📤 Enviando formulario con valores:", values)
    console.log("📤 Datos persistentes:", persistentData.current)
    ...
    try {
      ...
      // Usar los datos persistentes
      const finalData = persistentData.current
  ...
  const nextStep = () => {
    console.log("➡️ Avanzando al siguiente paso. Datos actuales:", persistentData.current)
  ...
  const prevStep = () => {
    console.log("⬅️ Retrocediendo al paso anterior. Datos actuales:", persistentData.current)
  ...
  const isStepComplete = () => {
    const currentData = persistentData.current
        ...
        return (
          <div className="space-y-6">
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      onChange={(e) => {
                        updatePersistentData("title", e.target.value)
                      }}
                    />
                  </FormControl>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      ...
                      onChange={(e) => {
                        updatePersistentData("description", e.target.value)
                      }}
                    />
                  </FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select
                      onValueChange={(value) => {
                        updatePersistentData("pipeline_stage_id", value)
                      }}
                      value={currentData.pipeline_stage_id}
            ...
            <div className="mt-4 p-3 bg-gray-100 rounded text-sm">
              ...
              <br />
              Title: {currentData.title || "vacío"}
              <br />
              Description: {currentData.description || "vacío"}
              <br />
              Stage: {currentData.pipeline_stage_id || "vacío"}
              <br />
        ...
        return (
          <div className="space-y-6">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select
                    ...
                    value={currentData.tech_company_id}
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select
                      ...
                      value={currentData.partner_id || undefined}
            ...
            <div className="mt-4 p-3 bg-gray-100 rounded text-sm">
              ...
              <br />
              Title (del paso 1): {currentData.title || "vacío"}
              <br />
              Tech Company: {currentData.tech_company_id || "vacío"}
              <br />
              Partner: {currentData.partner_id || "vacío"}
              <br />
              Completo: {isStepComplete() ? "✅" : "❌"}
            </div>
          </div>
        ...
        return (
          <div className="space-y-6">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <div className="flex gap-2">
                    <Select onValueChange={handleEndCustomerChange} value={currentData.end_customer_id || undefined}>
                      <FormControl>
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <div className="relative">
                        ...
                        <Input
                          type="number"
                          value={currentData.estimated_value === null ? "" : currentData.estimated_value}
                          onChange={(e) => {
                            const value = e.target.value === "" ? null : Number.parseFloat(e.target.value)
                            updatePersistentData("estimated_value", value)
                          }}
                          className="pl-10"
                          placeholder="0.00"
                        />
            ...
            <div className="mt-4 p-3 bg-gray-100 rounded text-sm">
              ...
              Title (del paso 1): {currentData.title || "vacío"}
              <br />
              Tech Company (del paso 2): {currentData.tech_company_id || "vacío"}
              <br />
              End Customer: {currentData.end_customer_id || "vacío"}
              <br />
              Estimated Value: {currentData.estimated_value || "vacío"}
              <br />
              Completo: {isStepComplete() ? "✅" : "❌"}
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 border rounded-md bg-blue-50 border-blue-100">
              ...
              <div className="space-y-2 text-sm text-blue-700">
                <p>
                  <strong>Título:</strong> {currentData.title || "No especificado"}
                </p>
                <p>
                  <strong>Descripción:</strong> {currentData.description || "No especificada"}
                </p>
                <p>
                  <strong>Empresa tecnológica:</strong>{" "}
                  {techCompanies.find((c) => c.id === currentData.tech_company_id)?.name || "No especificada"}
                </p>
                <p>
                  <strong>Partner:</strong>{" "}
                  {filteredPartners.find((p) => p.id === currentData.partner_id)?.name || "No especificado"}
                </p>
                <p>
                  <strong>Cliente final:</strong>{" "}
                  {endCustomers.find((c) => c.id === currentData.end_customer_id)?.name || "No especificado"}
                </p>
                <p>
                  <strong>Valor estimado:</strong> {currentData.estimated_value || "No especificado"}
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 p-3 bg-gray-100 rounded text-sm">
              ...
              <pre className="mt-2 text-xs overflow-auto">{JSON.stringify(currentData, null, 2)}</pre>
            </div>
          </div>
        )
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto shadow-lg border-0">
      <CardHeader className="bg-gradient-to-r from-primary/10 to-primary/5 border-b">
        <CardTitle className="text-2xl">Crear nueva oportunidad (FIXED)</CardTitle>
        <CardDescription>Versión con persistencia de datos usando useRef</CardDescription>
