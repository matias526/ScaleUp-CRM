...
export default function OpportunityCreateForm() {
  ...
  // Load existing prospect partners
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const filtered = existingProspectPartners.filter((p) =>
  ...
  const handleSelectExistingProspect = async (prospect: any) => {
    ...
    try {
      ...
      if (contacts && contacts.length > 0) {
        // Set the first contact as the contact data
        const firstContact = contacts[0]
        setProspectContactData({
          first_name: firstContact.first_name || "",
          last_name: firstContact.last_name || "",
          email: firstContact.email || "",
          phone: firstContact.phone || "",
          preferred_language: (firstContact.preferred_language || "es") as "es" | "en" | "pt",
        })
      }
    } catch (err) {
                    ...
                    <div className="space-y-6 mt-6">
                      ...
                      {!form.watch("is_prospect") && isScaleUpUser && (
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>{t("opportunities.form.partner")} ({t("opportunities.form.optional")})</FormLabel>
              ...
              <div className="space-y-2 relative">
                ...
                <div className="relative">
                  <Input
                    ...
                    value={prospectPartnerData.name || prospectSearchQuery}
                    onChange={(e) => {
                      const value = e.target.value
                      setProspectSearchQuery(value)
                      setProspectPartnerData({ ...prospectPartnerData, name: value })
                    }}
