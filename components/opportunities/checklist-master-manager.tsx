...
export function ChecklistMasterManager() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const { data, error } = await supabase.from("checklist_master_items").select("*").eq("target_type", target).order("order_index", { ascending: true })
  ...
  const activeWeight = useMemo(() => items.filter((item) => item.is_active).reduce((sum, item) => sum + Number(item.weight || 0), 0), [items])
  ...
  const save = async () => {
    ...
    const result = editing ? await supabase.from("checklist_master_items").update(payload).eq("id", editing.id) : await supabase.from("checklist_master_items").insert(payload)
    if (result.error) toast.error(result.error.message); else { toast.success("Ítem guardado"); setOpen(false); await load() }
    setSaving(false)
  }
  const remove = async () => { if (!deleteId) return; const childResult = await supabase.from("checklist_master_items").delete().eq("parent_id", deleteId); if (childResult.error) return toast.error(childResult.error.message); const { error } = await supabase.from("checklist_master_items").delete().eq("id", deleteId); if (error) toast.error(error.message); else { toast.success("Ítem eliminado"); await load() }; setDeleteId(null) }
