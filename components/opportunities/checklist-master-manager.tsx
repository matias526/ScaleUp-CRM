...
export function ChecklistMasterManager() {
  ...
  const groupTotals = useMemo(() => {
    ...
    return {
      root: totalFor(null),
      parents: items
        .filter((item) => !item.parent_id && items.some((child) => child.parent_id === item.id))
        .map((item) => ({ id: item.id, title: item.title.es || item.title.en, total: totalFor(item.id) })),
    }
  }, [items])
