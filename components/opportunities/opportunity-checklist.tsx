  ...
  const renderItem = (item: ChecklistItem, level = 0) => {
    ...
    return <div key={item.id} className="border-b last:border-b-0">
      <div className="flex items-start gap-3 py-3" style={{ paddingLeft: `${level * 1.25}rem` }}>
        ...
        <div className="min-w-0 flex-1 cursor-pointer" onClick={() => isRoot && nested.length ? toggleRoot(item.id) : !item.is_completed && selectItem(item)}><div className="flex items-center gap-2"><p className="font-medium">{title(item)}</p>{item.is_completed && <Badge variant="default" className="bg-emerald-600 hover:bg-emerald-600">Completado</Badge>}</div><p className="text-sm text-muted-foreground">{description(item)}</p>{item.is_completed && <div className="mt-2 flex flex-col gap-1 text-xs text-muted-foreground"><span>Fecha de realización: {item.user_selected_date ? new Date(`${item.user_selected_date}T00:00:00`).toLocaleDateString("es-AR") : "Sin fecha"}</span>{item.comment && <span>Comentario: {item.comment}</span>}</div>}</div>
