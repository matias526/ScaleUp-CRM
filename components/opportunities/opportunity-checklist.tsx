    ...
    <Card className="mb-5 border-muted shadow-none">
      <CardContent className="px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex h-10 w-full overflow-hidden rounded-md border border-border bg-muted/40">{roots.map((root) => { const rootChildren = children(root.id); const rootProgress = rootChildren.length ? calculateProgress([root, ...rootChildren]) : root.is_completed ? 100 : 0; const fillColor = rootProgress === 100 ? "bg-emerald-500" : rootProgress > 0 ? "bg-amber-400" : "bg-muted-foreground/20"; return <button key={root.id} type="button" title={`${title(root)} · ${rootProgress}%`} className="relative h-full min-w-0 border-r border-background text-left transition-opacity hover:opacity-90 last:border-r-0" style={{ width: `${root.weight}%` }} onClick={() => { if (rootChildren.length) toggleRoot(root.id); else selectItem(root) }}><span className={`absolute inset-y-0 left-0 ${fillColor} transition-all`} style={{ width: `${rootProgress}%` }} /><span className="relative z-10 flex h-full items-center truncate px-2 text-[11px] font-medium text-foreground">{title(root)}</span></button> })}</div>
        </div>
        </div>
