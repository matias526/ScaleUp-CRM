  ...
  return <>
    <Card className="mb-5 overflow-hidden border-border/70 shadow-none">
      <CardContent className="px-4 py-4 sm:px-5">
        ...
        <div className="flex h-12 w-full overflow-hidden rounded-lg border border-border/80 bg-muted/35 shadow-inner">{roots.map((root) => { const rootChildren = children(root.id); const rootProgress = rootChildren.length ? calculateProgress([root, ...rootChildren]) : root.is_completed ? 100 : 0; const fillColor = rootProgress === 100 ? "bg-emerald-500/85" : rootProgress > 0 ? "bg-amber-400/90" : "bg-muted-foreground/10"; return <button key={root.id} type="button" title={`${title(root)} · ${rootProgress}%`} className="relative h-full min-w-0 border-r border-background/80 text-left transition-[filter] hover:brightness-95 last:border-r-0" style={{ width: `${root.weight}%` }} onClick={() => { if (rootChildren.length) toggleRoot(root.id); else selectItem(root) }}><span className={`absolute inset-y-0 left-0 ${fillColor} transition-[width]`} style={{ width: `${rootProgress}%` }} /><span className="relative z-10 flex h-full items-center truncate px-2.5 text-[11px] font-medium text-foreground sm:text-xs">{title(root)}</span></button> })}</div>
        <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground"><span>Haz clic en una etapa para ver sus actividades</span><span className="tabular-nums">{items.filter((item) => item.is_completed).length}/{items.length}</span></div>
