...
export function MeetingDashboard({ opportunities, projections = [], isLoading, blank = false, partnerName = "", benchmarkPartners = [] }: MeetingDashboardProps) {
  ...
  if (blank) {
    ...
    const benchmarkRows = [...benchmarkPartners.map((partner) => ({ name: partner.name, value: partner.name === partnerName ? currentMetricValue : metricValue(partner) })), ...(partnerName && !benchmarkPartners.some((partner) => partner.name === partnerName) ? [{ name: partnerName, value: currentMetricValue }] : [])].sort((a, b) => b.value - a.value || a.name.localeCompare(b.name))
    ...
    const visibleBenchmarkRows = [...hiddenPartnerNames[rankingMetric].map((name, index) => ({ name, value: 0, position: index + 1, hidden: true })), ...benchmarkRows.slice(0, 5).map((row, index) => ({ ...row, position: index + 3, hidden: false }))]
