...
export function MeetingDashboard({ opportunities, projections = [], isLoading, blank = false }: MeetingDashboardProps) {
  ...
  if (blank) {
    ...
    const openOpportunities = opportunities.filter((opportunity) => !["won", "closed_won", "closed won", "lost", "closed_lost", "closed lost"].includes(String(opportunity.pipeline_stage?.code ?? opportunity.stage ?? opportunity.status ?? "").toLowerCase()))
    ...
    const quarterWon = (quarter: number) => opportunities.filter((opportunity) => { const date = opportunityCloseDate(opportunity); const stage = String(opportunity.pipeline_stage?.code ?? opportunity.stage ?? opportunity.status ?? "").toLowerCase(); return date && Math.ceil((new Date(date).getUTCMonth() + 1) / 3) === quarter && ["won", "closed_won", "closed won"].includes(stage) }).reduce((sum, opportunity) => sum + opportunityAmount(opportunity), 0)
    const quarterWeighted = (quarter: number) => opportunities.filter((opportunity) => { const date = opportunityCloseDate(opportunity); const stage = String(opportunity.pipeline_stage?.code ?? opportunity.stage ?? opportunity.status ?? "").toLowerCase(); return date && Math.ceil((new Date(date).getUTCMonth() + 1) / 3) === quarter && !["won", "closed_won", "closed won", "lost", "closed_lost", "closed lost"].includes(stage) }).reduce((sum, opportunity) => sum + opportunityAmount(opportunity) * (Number(opportunity.probability ?? opportunity.pipeline_stage?.probability ?? 0) / 100), 0)
    const quarters = [1, 2, 3, 4].map((quarter) => { const quarterTargetValue = quarterTarget(quarter); const wonValue = quarterWon(quarter); const weightedValue = quarterWeighted(quarter); return { label: `Q${quarter}`, won: wonValue, weighted: weightedValue, target: quarterTargetValue, status: wonValue >= quarterTargetValue && quarterTargetValue > 0 ? "On target" : weightedValue > 0 ? "In progress" : "No pipeline" } })
    ...
    const actualWonCount = opportunities.filter((opportunity) => ["won", "closed_won", "closed won"].includes(String(opportunity.pipeline_stage?.code ?? opportunity.stage ?? opportunity.status ?? "").toLowerCase())).length
    const actualProposalCount = opportunities.filter((opportunity) => ["quotation", "proposal"].includes(String(opportunity.pipeline_stage?.code ?? opportunity.stage ?? opportunity.status ?? "").toLowerCase())).length
    const hygiene = [
      { icon: CalendarClock, label: "Without close date", value: openOpportunities.filter((opportunity) => !opportunityCloseDate(opportunity)).length, detail: "Deals need an estimated close date" },
      { icon: Gauge, label: "Low probability (<50%)", value: openOpportunities.filter((opportunity) => Number(opportunity.probability ?? opportunity.win_probability ?? 0) < 50).length, detail: "Deals still in early stage" },
      ...
      { icon: TrendingUp, label: "Open pipeline value", value: money(openOpportunities.reduce((sum, opportunity) => sum + opportunityAmount(opportunity), 0)), detail: "Total unweighted pipeline" },
    ]
