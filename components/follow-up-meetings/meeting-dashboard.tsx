...
export function MeetingDashboard({ opportunities, projections = [], isLoading, blank = false, partnerName = "", benchmarkPartners = [] }: MeetingDashboardProps) {
  ...
  if (blank) {
    ...
    const benchmarkRows = [...benchmarkPartners.map((partner) => ({ name: partner.name, value: metricValue(partner) })), ...(partnerName && !benchmarkPartners.some((partner) => partner.name === partnerName) ? [{ name: partnerName, value: currentMetricValue }] : [])].sort((a, b) => b.value - a.value)
