...
export function MeetingDashboard({ opportunities, projections = [], isLoading, blank = false }: MeetingDashboardProps) {
  ...
  if (blank) {
    ...
    const hygiene = [
      { icon: CalendarClock, label: "Without close date", value: openCurrentYearOpportunities.filter((opportunity) => !opportunityCloseDate(opportunity)).length, detail: "Open deals need an estimated close date" },
      { icon: Gauge, label: "Low probability (<50%)", value: openOpportunities.filter((opportunity) => Number(opportunity.probability ?? opportunity.win_probability ?? opportunity.pipeline_stage?.probability ?? 0) < 50).length, detail: "Current-year open deals" },
      { icon: Clock, label: "Stalled (>90 days)", value: openOpportunities.filter((opportunity) => opportunity.updated_at && Date.now() - new Date(opportunity.updated_at).getTime() > 90 * 86400000).length, detail: "Current-year open deals without recent movement" },
      { icon: TrendingUp, label: "Open pipeline value", value: money(openOpportunities.reduce((sum, opportunity) => sum + opportunityAmount(opportunity), 0)), detail: "Current-year unweighted pipeline" },
    ]
