...
const MEETING_DASHBOARD_TRANSLATIONS = {
  ...
  "dashboard.networkBenchmark": { es: "Comparativa con otros Partners", en: "Comparison with other partners", pt: "Comparação com outros parceiros" },
  ...
  "dashboard.dealCount": { es: "Cantidad de oportunidades", en: "Opportunity count", pt: "Quantidade de oportunidades" },
  "dashboard.outOfList": { es: "Fuera de lista", en: "Outside the list", pt: "Fora da lista" },
  "dashboard.noOperations": { es: "Aún no realizó operaciones", en: "No operations yet", pt: "Ainda não realizou operações" },
...
export function MeetingDashboard({ opportunities, projections = [], isLoading, blank = false, partnerName = "", benchmarkPartners = [] }: MeetingDashboardProps) {
  ...
  if (blank) {
    ...
    const annualCreatedOpportunities = opportunities.filter((opportunity) => { const date = opportunity.created_at ? new Date(opportunity.created_at) : null; return date && date.getFullYear() === currentYear })
    const annualWonOpportunities = opportunities.filter((opportunity) => { const updatedAt = opportunity.updated_at ? new Date(opportunity.updated_at) : null; return isWon(opportunity) && updatedAt && !Number.isNaN(updatedAt.getTime()) && updatedAt.getFullYear() === currentYear })
    const currentMetricValue = rankingMetric === "closed" ? annualWonOpportunities.reduce((sum, opportunity) => sum + amount(opportunity), 0) : rankingMetric === "target" ? (target > 0 ? won / target * 100 : 0) : rankingMetric === "deals" ? annualCreatedOpportunities.length : (() => { const closed = annualWonOpportunities.filter((opportunity) => opportunity.created_at); return closed.length ? closed.reduce((sum, opportunity) => sum + (new Date(opportunity.updated_at).getTime() - new Date(opportunity.created_at).getTime()) / 86400000, 0) / closed.length : 0 })()
    const metricValue = (partner: any) => Number(partner[rankingMetric === "closed" ? "closed_amount" : rankingMetric === "target" ? "target_attainment" : rankingMetric === "deals" ? "deal_count" : "close_speed"] ?? 0)
    const benchmarkRows = [...benchmarkPartners.map((partner) => ({ name: partner.name, value: partner.name === partnerName ? currentMetricValue : metricValue(partner) })), ...(partnerName && !benchmarkPartners.some((partner) => partner.name === partnerName) ? [{ name: partnerName, value: currentMetricValue }] : [])].sort((a, b) => rankingMetric === "speed" ? a.value - b.value || a.name.localeCompare(b.name) : b.value - a.value || a.name.localeCompare(b.name))
    const calculatedPosition = benchmarkRows.findIndex((row) => row.name === partnerName) + 1
    const currentPosition = calculatedPosition > 0 ? calculatedPosition + 2 : 0
    const hiddenPartnerNames: Record<string, [string, string]> = { closed: ["Partner Norte", "Partner Centro"], target: ["Partner Atlántico", "Partner Pacífico"], deals: ["Partner Delta", "Partner Sigma"], speed: ["Partner Nova", "Partner Horizonte"] }
    const rankedRealRows = benchmarkRows.filter((row) => row.name !== partnerName).slice(0, 4)
    const currentRow = benchmarkRows.find((row) => row.name === partnerName)
    const displayedRealRows = currentRow && !rankedRealRows.some((row) => row.name === partnerName) ? [...rankedRealRows, currentRow] : rankedRealRows
    const visibleBenchmarkRows = [...hiddenPartnerNames[rankingMetric].map((name, index) => ({ name, value: 0, position: index + 1, hidden: true })), ...displayedRealRows.map((row) => ({ ...row, position: benchmarkRows.findIndex((rankedRow) => rankedRow.name === row.name) + 3, hidden: false }))].sort((a, b) => a.position - b.position)
    const benchmarkValue = (value: number) => value === 0 ? t("dashboard.outOfList") : rankingMetric === "closed" ? money(value) : rankingMetric === "target" ? `${value.toFixed(0)}%` : rankingMetric === "speed" ? `${value.toFixed(0)}d` : value.toString()
    return (
      <Card className="mx-auto min-h-[560px] w-full overflow-hidden border-2 border-primary/20 shadow-sm md:w-3/5">
        <CardHeader className="border-b bg-gradient-to-r from-gray-50 to-gray-100 pb-3">
          <div className="flex items-start justify-between gap-3"><div><CardTitle className="text-xl font-bold text-gray-900">{partnerName || t("dashboard.partnerAccount")}</CardTitle><p className="mt-1 text-xs text-gray-500">{t("dashboard.annualSubtitle")}</p></div><BarChart3 className="h-5 w-5 text-indigo-500" /></div>
        </CardHeader>
        <CardContent className="space-y-4 p-4 text-sm">
          ...
          <div className="mb-3 grid grid-cols-3 gap-2">{[{ label: "Declared", target: projectionTotals.declared, actual: opportunities.filter((opportunity) => Boolean(opportunity.created_at)).length }, { label: "Proposal", target: projectionTotals.proposal, actual: actualProposalCount }, { label: "Won", target: projectionTotals.won, actual: actualWonCount }].map((metric) => <div key={metric.label} className="rounded-md border bg-white p-2"><div className="text-xs font-semibold text-gray-600">{metric.label} opportunities</div><div className="mt-1 flex items-end justify-between"><span className="text-lg font-bold text-gray-900">{metric.actual}</span><span className="text-xs text-gray-500">target {metric.target}</span></div><Progress value={metric.target ? Math.min(100, metric.actual / metric.target * 100) : 0} className="mt-1 h-1.5" indicatorClassName="bg-indigo-500" /></div>)}</div><div className="grid grid-cols-4 gap-2">{quarters.map((quarter) => <div key={quarter.label} className="rounded-md border bg-white p-2"><div className="flex items-center justify-between"><div className="font-semibold text-gray-800">{quarter.label}</div><span className="text-xs text-gray-500">{t("dashboard.target")} {money(quarter.target)}</span></div><div className="mt-2 space-y-1.5"><div className="flex items-center justify-between"><span className="text-gray-500">{t("dashboard.declared")}</span><span className="font-semibold text-gray-800">{quarter.metrics.declared} <span className="font-normal text-gray-400">({quarter.metrics.declaredTarget})</span></span></div><div className="flex items-center justify-between"><span className="text-gray-500">{t("dashboard.proposal")}</span><span className="font-semibold text-gray-800">{quarter.metrics.proposal} <span className="font-normal text-gray-400">({quarter.metrics.proposalTarget})</span></span></div><div className="flex items-center justify-between"><span className="text-gray-500">{t("dashboard.won")}</span><span className="font-semibold text-emerald-700">{quarter.metrics.won} <span className="font-normal text-emerald-600">({quarter.metrics.wonTarget})</span></span></div></div><Progress value={quarter.target ? Math.min(100, (quarter.won + quarter.weighted) / quarter.target * 100) : 0} className="mt-2 h-1.5" indicatorClassName="bg-blue-500" /><div className="mt-1 text-xs text-gray-500">{quarter.status === "On target" ? t("dashboard.onTarget") : quarter.status === "In progress" ? t("dashboard.inProgress") : t("dashboard.noPipeline")}</div></div>)}</div>
          <section><div className="mb-2 flex items-center gap-1 font-semibold text-gray-700"><Filter className="h-3.5 w-3.5 text-amber-500" />{t("dashboard.pipelineHygiene")}</div><div className="mb-3 grid gap-3 rounded-xl border border-primary/15 bg-gradient-to-br from-card via-card to-primary/[0.04] p-3 shadow-sm md:grid-cols-5"><div className="md:col-span-5 mb-0 flex items-start justify-between"><div><p className="text-xs font-semibold uppercase tracking-wide text-primary/75">{t("dashboard.openOpportunities")}</p><p className="mt-1 text-2xl font-bold tracking-tight text-gray-900">{openOpportunities.length} <span className="text-xs font-normal text-gray-500">{t("dashboard.inPipeline")}</span></p></div><p className="rounded-full bg-primary/[0.08] px-2 py-1 text-xs font-medium text-primary">{t("dashboard.opportunityProbability")}</p></div><div className="md:col-span-3 flex flex-col gap-1.5 border-b border-border/70 pb-3">{probabilityBuckets.map((bucket) => <div key={bucket.label} className="flex items-center gap-2"><span className={`flex h-6 w-14 shrink-0 items-center justify-center rounded-md text-xs font-bold text-white ${bucket.tone}`}>{bucket.label}</span><div className="relative h-7 flex-1 overflow-hidden rounded-r-lg bg-muted/50"><div className={`absolute inset-y-0 left-0 ${bucket.tone}`} style={{ width: `${bucket.count / Math.max(openOpportunities.length, 1) * 100}%` }} /><div className="relative z-10 flex h-full items-center justify-between px-2 text-xs font-semibold text-white"><span>{bucket.count} opportunities</span><span>{bucket.amount}</span></div></div></div>)}</div><div className="md:col-span-2 mt-0 grid grid-cols-2 gap-2"><div className="rounded-xl border border-primary/10 bg-gradient-to-br from-primary/[0.05] to-card p-2 text-center"><p className="text-xs text-gray-500">Closing next 30 days</p><p className="mt-1 text-lg font-semibold text-gray-900">{money(closing30.reduce((sum, opportunity) => sum + opportunityAmount(opportunity), 0))}</p><p className="text-xs text-gray-500">Weighted amount</p></div><div className="rounded-xl border border-sky-200/70 bg-gradient-to-br from-sky-50/80 to-card p-2 text-center"><p className="text-xs font-medium text-sky-700">Closing next 90 days</p><p className="mt-1 text-lg font-semibold text-gray-900">{money(weighted90)}</p><p className="text-xs text-gray-500">Weighted amount</p></div></div></div><div className="md:col-span-5 grid grid-cols-2 gap-2">{hygiene.map(({ icon: Icon, label, value, detail }) => <div key={label} className="rounded-md border border-amber-100 bg-amber-50/70 p-2"><Icon className="h-4 w-4 text-amber-600" /><div className="mt-1 font-semibold text-gray-800">{value} {label}</div><p className="mt-0.5 text-xs text-gray-500">{detail}</p></div>)}</div></section>
          <section><div className="mb-2 flex items-center gap-1 font-semibold text-gray-700"><Trophy className="h-3.5 w-3.5 text-indigo-500" />{t("dashboard.networkBenchmark")}</div><div className="mb-2 flex flex-wrap gap-1">{Object.entries(rankingLabels).map(([key, label]) => <button key={key} type="button" onClick={() => setRankingMetric(key)} className={`rounded border px-2 py-1 text-xs ${rankingMetric === key ? "border-primary bg-primary/10 font-semibold text-primary" : "bg-white text-gray-500"}`}>{label}</button>)}</div><div className="overflow-hidden rounded-md border"><div className="grid grid-cols-[1fr_auto] bg-gray-50 px-2 py-1 font-semibold"><span>{rankingLabels[rankingMetric]}</span><span>{t("dashboard.score")}</span></div>{visibleBenchmarkRows.length > 2 ? visibleBenchmarkRows.map((row) => <div key={`${rankingMetric}-${row.name}`} className={`grid grid-cols-[1fr_auto] px-2 py-1 ${row.name === partnerName ? "bg-primary/10 font-bold" : ""}`}><span className={row.hidden ? "blur-[7px] select-none pointer-events-none opacity-60" : row.name === partnerName ? "" : "blur-[7px] select-none pointer-events-none opacity-60"}>{row.name} {row.name === partnerName && currentMetricValue === 0 ? "" : `- #${row.position}`}</span><span className={row.hidden ? "blur-[7px] select-none pointer-events-none opacity-60" : row.name === partnerName ? "" : "blur-[7px] select-none pointer-events-none opacity-60"}>{row.name === partnerName && currentMetricValue === 0 ? `${t("dashboard.outOfList")} — ${t("dashboard.noOperations")}` : row.name === partnerName ? benchmarkValue(currentMetricValue) : "•••"}</span></div>) : <div className="px-2 py-2 text-gray-500">{t("dashboard.noBenchmarkData")}</div>}</div></section>
        </CardContent>
      </Card>
    )
  }
