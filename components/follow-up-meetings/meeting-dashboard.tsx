...
export function MeetingDashboard({ opportunities, projections = [], isLoading, blank = false }: MeetingDashboardProps) {
  ...
  if (blank) {
    ...
    const quarterMetrics = (quarter: number) => {
      ...
      return { declared: declaredDeals.length, declaredAmount: declaredDeals.reduce((sum, opportunity) => sum + amount(opportunity), 0), proposal: proposalDeals.length, proposalAmount: proposalDeals.reduce((sum, opportunity) => sum + amount(opportunity), 0), won: wonDeals.length, wonAmount: wonDeals.reduce((sum, opportunity) => sum + amount(opportunity), 0) }
    }
