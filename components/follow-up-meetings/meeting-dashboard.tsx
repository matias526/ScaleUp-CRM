    ...
    return (
      <Card className="mx-auto min-h-[560px] w-full overflow-hidden border-2 border-primary/20 shadow-sm md:w-3/5">
        ...
        <CardContent className="space-y-4 p-4 text-xs">
          ...
          <section className="rounded-lg border bg-white p-3"><div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-1.5 font-semibold text-gray-700"><BarChart3 className="h-3.5 w-3.5 text-blue-500" />Opportunities</div><span className="text-[10px] text-gray-400">Mockup</span></div><div className="grid grid-cols-3 gap-2"><div className="rounded-md bg-slate-50 p-2"><div className="mb-2 flex items-start justify-between gap-1"><span className="text-[10px] font-medium text-gray-600">Total opportunities</span><span className="text-sm font-bold text-slate-900">24</span></div><div className="flex h-14 items-end gap-1">{[42, 56, 48, 68, 62, 78, 72, 90].map((height, index) => <div key={index} className="flex-1 rounded-t bg-blue-400/80" style={{ height: `${height}%` }} />)}</div><p className="mt-1 text-[10px] text-gray-400">Pipeline by stage</p></div><div className="rounded-md bg-emerald-50/60 p-2"><div className="mb-2 flex items-start justify-between gap-1"><span className="text-[10px] font-medium text-gray-600">Closing next 30 days</span><span className="text-sm font-bold text-emerald-700">$18.4K</span></div><div className="flex h-14 items-end gap-1">{[28, 40, 52, 46, 72, 64, 84, 92].map((height, index) => <div key={index} className="flex-1 rounded-t bg-emerald-500/80" style={{ height: `${height}%` }} />)}</div><p className="mt-1 text-[10px] text-gray-400">6 opportunities</p></div><div className="rounded-md bg-indigo-50/60 p-2"><div className="mb-2 flex items-start justify-between gap-1"><span className="text-[10px] font-medium text-gray-600">Weighted pipeline · 90 days</span><span className="text-sm font-bold text-indigo-700">$42.7K</span></div><div className="flex h-14 items-end gap-1">{[35, 48, 44, 60, 58, 76, 70, 88].map((height, index) => <div key={index} className="flex-1 rounded-t bg-indigo-500/80" style={{ height: `${height}%` }} />)}</div><p className="mt-1 text-[10px] text-gray-400">Based on probability</p></div></div></section>
          ...
          <section><div className="mb-2 flex items-center gap-1 font-semibold text-gray-700"><AlertCircle className="h-3.5 w-3.5 text-amber-500" />Pipeline hygiene</div><div className="grid grid-cols-3 gap-2">{hygiene.map(({ icon: Icon, label, value, detail }) => <div key={label} className="rounded-md border border-amber-100 bg-amber-50/70 p-2"><Icon className="h-4 w-4 text-amber-600" /><div className="mt-1 font-semibold text-gray-800">{value} {label}</div><p className="mt-0.5 text-[10px] text-gray-500">{detail}</p></div>)}</div></section>
