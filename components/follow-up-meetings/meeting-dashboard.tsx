...
export function MeetingDashboard({ opportunities, projections = [], isLoading, blank = false, partnerName = "", benchmarkPartners = [] }: MeetingDashboardProps) {
  ...
  if (blank) {
    ...
    const visibleBenchmarkRows = [...hiddenPartnerNames[rankingMetric].map((name, index) => ({ name, value: 0, position: index + 1, hidden: true })), ...displayedRealRows.map((row) => ({ ...row, position: row.name === partnerName ? currentPosition : benchmarkRows.findIndex((rankedRow) => rankedRow.name === row.name) + 3, hidden: false }))]
