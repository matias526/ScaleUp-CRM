...
export function MeetingDashboard({ opportunities, projections = [], isLoading, blank = false, partnerName = "", benchmarkPartners = [] }: MeetingDashboardProps) {
  ...
  if (blank) {
    ...
    const annualWonOpportunities = opportunities.filter((opportunity) => isWon(opportunity) && opportunity.updated_at && new Date(opportunity.updated_at).getFullYear() === currentYear)
