...
export function MeetingDashboard({ opportunities, projections = [], isLoading, blank = false, partnerName = "", benchmarkPartners = [] }: MeetingDashboardProps) {
  ...
  if (blank) {
    ...
    const currentMetricValue = rankingMetric === "closed" ? won : rankingMetric === "target" ? (target > 0 ? won / target * 100 : 0) : rankingMetric === "deals" ? opportunities.length : (() => { const closed = opportunities.filter(isWon).filter((opportunity) => opportunity.created_at && opportunity.updated_at); return closed.length ? closed.reduce((sum, opportunity) => sum + (new Date(opportunity.updated_at).getTime() - new Date(opportunity.created_at).getTime()) / 86400000, 0) / closed.length : 0 })()
    ...
    const benchmarkValue = (value: number) => rankingMetric === "closed" ? money(value) : rankingMetric === "target" ? `${value.toFixed(0)}%` : rankingMetric === "speed" ? `${value.toFixed(0)}d` : value.toString()
    return (
