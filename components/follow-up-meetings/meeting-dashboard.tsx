...
export function MeetingDashboard({ opportunities, projections = [], isLoading, blank = false }: MeetingDashboardProps) {
  ...
  if (blank) {
    ...
    const isOpen = (opportunity: any) => !closedStates.has(stage(opportunity))
    ...
    const quarterMetrics = (quarter: number) => {
      const inQuarter = opportunities.filter((opportunity) => { const date = opportunityCloseDate(opportunity); return date && Math.floor(new Date(date).getMonth() / 3) + 1 === quarter })
      const proposal = inQuarter.filter((opportunity) => ["quotation", "proposal"].includes(normalizeStatus(opportunity.pipeline_stage?.code ?? opportunity.stage ?? opportunity.status)))
      const wonDeals = inQuarter.filter(isWon)
      return { declared: inQuarter.length, declaredAmount: inQuarter.reduce((sum, opportunity) => sum + amount(opportunity), 0), proposal: proposal.length, proposalAmount: proposal.reduce((sum, opportunity) => sum + amount(opportunity), 0), won: wonDeals.length, wonAmount: wonDeals.reduce((sum, opportunity) => sum + amount(opportunity), 0) }
    }
