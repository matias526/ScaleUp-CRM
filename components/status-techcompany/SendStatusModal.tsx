...
export function SendStatusModal({ open, onOpenChange, techCompanyName, techCompanyId, techCompanyLogo, year, partners, inProcessPartners, onCancel }: SendStatusModalProps) {
  ...
  const sendEmail = async () => {
    ...
    const html = `<div style="font-family:Arial,sans-serif;color:#172033"><div style="display:flex;align-items:center;gap:20px;border-bottom:1px solid #dbe2ea;padding:18px 0"><img src="${window.location.origin}/images/scaleup-logo-color.png" alt="ScaleUp" height="36" /><img src="${techCompanyLogo || `${window.location.origin}/placeholder-logo.svg`}" alt="${techCompanyName}" height="36" /><strong>${techCompanyName}</strong></div><h1>Weekly Status - ${techCompanyName} - W37 - ${year}</h1><p>${note || "Weekly business status update"}</p><h2>${tx("send.forecast")}</h2><p>${tx("send.annualTarget")}: ${money(totals.target)} · ${tx("send.wonYtd")}: ${money(totals.won)} · ${tx("send.weighted90")}: ${money(totals.pipeline)}</p><h2>${tx("send.partners")}</h2><ul>${partners.map((partner) => `<li>${partner.name} · ${partner.countryName ?? "—"} · ${statusLabel(partner.status)}</li>`).join("")}</ul></div>`
