...
const STATUS_MODAL_TRANSLATIONS = {
  ...
  "send.sent": { es: "Email enviado", en: "Email sent", pt: "E-mail enviado" },
  "send.emailDescription": { es: "El reporte fue enviado correctamente.", en: "The report was sent successfully.", pt: "O relatório foi enviado com sucesso." },
...
export function SendStatusModal({ open, onOpenChange, techCompanyName, techCompanyId, year, partners, unassignedWon, inProcessPartners, onCancel }: SendStatusModalProps) {
  ...
  const overallStatusKey = totals.target > 0 && expected >= totals.target ? "status.onTrack" : "status.needsFocus"
  const overallStatus = tx(overallStatusKey)
  ...
  const downloadPdf = async () => {
    ...
    pdf.save(`weekly-status-${techCompanyName.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}-${year}.pdf`)
    return

    const legacyPdf = new jsPDF({ unit: "pt", format: "a4" })
    const legacyMargin = 42
    const legacyPageWidth = pdf.internal.pageSize.getWidth()
    let y = 48
    const addText = (text: string, size = 10, bold = false) => { pdf.setFont("helvetica", bold ? "bold" : "normal"); pdf.setFontSize(size); const lines = pdf.splitTextToSize(text, legacyPageWidth - legacyMargin * 2); pdf.text(lines, legacyMargin, y); y += lines.length * (size + 4) + 6 }
    const divider = () => { pdf.setDrawColor(220, 226, 234); pdf.line(legacyMargin, y, legacyPageWidth - legacyMargin, y); y += 18 }
    const ensureSpace = (height = 42) => { if (y + height > pdf.internal.pageSize.getHeight() - 42) { pdf.addPage(); y = 48 } }
    addText(`Weekly Status - ${techCompanyName} - W37 - ${year}`, 20, true)
    addText(`${tx("send.generated")}: ${new Date().toLocaleDateString("en-US")} (${tx("send.snapshot")}) | ${tx("send.state")}: ${overallStatus} (${expectedPercent}% ${tx("send.annualCoverage")})`, 10)
    divider()
    addText(`1. ${tx("send.forecast")} (${tx("send.annualTarget")}: ${money(totals.target)})`, 11, true)
    addText(`${tx("send.annualTarget")}: ${money(totals.target)} (100% ${tx("send.goal")})`)
    addText(`${tx("send.wonYtd")}: ${money(totals.won)} (${coverage}% ${tx("send.coverage")})`)
    addText(`${tx("send.weighted90")}: ${money(totals.pipeline)} (${totals.target ? Math.round(totals.pipeline / totals.target * 100) : 0}% ${tx("send.additional")})`)
    addText(`${tx("send.expectedTotal")}: ${money(expected)} / ${money(totals.target)} (${expectedPercent}% ${tx("send.coverage")})    ${tx("send.remainingGap")}: ${money(remaining)}`, 10, true)
    divider()
    addText(`2. ${tx("send.partners")}`, 11, true)
    addText(`${tx("send.partner")} | ${tx("send.country")} | Won | Q3/Q4 Pipeline | Forecast`, 9, true)
    partners.forEach((partner) => { ensureSpace(); addText(`${partner.name} | ${partner.countryName ?? "—"} | ${money(partner.won)} | ${money(partner.pipeline)} | ${money(partner.target)}`, 9) })
    divider()
    addText(`3. ${tx("send.hot")} (Next 90 Days)`, 11, true)
    addText(`${tx("send.clientDeal")} | ${tx("send.partner")} | Client | ${tx("send.amount")} | ${tx("send.probability")} | ${tx("send.estimatedClose")}`, 9, true)
    opportunities.forEach((opportunity) => { ensureSpace(); addText(`${opportunity.title} | ${opportunity.partner} | ${opportunity.customerName} | ${money(opportunity.amount)} | ${opportunity.probability}% | ${date(opportunity.closeDate, "en")}`, 9) })
    divider()
    addText(`4. ${tx("send.criticalActions")}`, 11, true)
    impacts.forEach((impact) => { ensureSpace(); addText(`${impact.title} — ${impact.partner}${impact.scope ? ` · ${impact.scope}` : ""}: ${impact.description || tx("send.requiresFollowUp")} (${tx("send.financialImpact")}: ${money(impact.amount)})`, 9) })
    divider()
    addText(`5. ${tx("send.expansion")}`, 11, true)
    inProcessPartners.filter((partner) => partner.probability > 40).forEach((partner) => { ensureSpace(); addText(`${partner.name} | ${partner.country} | ${partner.probability}% | ${date(partner.closeDate, "en")}`, 9) })
    pdf.save(`weekly-status-${techCompanyName.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}-${year}.pdf`)
  }
  ...
  const sendEmail = async () => {
    ...
    const html = `<div style="background:#f3f4f6;padding:28px 12px;font-family:Arial,sans-serif;color:#172033"><article style="max-width:820px;margin:0 auto;background:#fff;border:1px solid #dbe2ea;padding:44px;box-sizing:border-box"><table role="presentation" style="width:100%;border-collapse:collapse;border-bottom:1px solid #dbe2ea;padding-bottom:20px"><tr><td style="padding:0 16px 20px 0;vertical-align:top"><h1 style="margin:0;font-size:22px;line-height:1.25">Weekly Status - ${techCompanyName} - W37 - ${year}</h1><p style="margin:7px 0 0;color:#667085;font-size:13px;line-height:1.4">${tx("send.generated")}: ${reportDate} (${tx("send.snapshot")}) · ${tx("send.state")}: ${overallStatus} (${expectedPercent}% ${tx("send.annualCoverage")})</p></td><td style="padding:0 0 20px;text-align:right;vertical-align:top;width:150px"><img src="${scaleUpLogo}" alt="ScaleUp" style="display:block;margin-left:auto;height:38px;width:auto" /></td></tr></table>${note ? `<div style="margin-top:24px;padding:14px 16px;background:#fffbea;border:1px solid #f2d675;border-radius:6px;font-size:14px">${note}</div>` : ""}<section style="padding:26px 0;border-bottom:1px solid #dbe2ea"><h2 style="font-size:13px;text-transform:uppercase;letter-spacing:.08em;color:#667085">1. ${tx("send.forecast")}</h2>${progressRow(tx("send.annualTarget"), totals.target, `100% ${tx("send.goal")}`, "#98a2b3")}${progressRow(tx("send.wonYtd"), totals.won, `${coverage}% ${tx("send.coverage")}`, "#10b981")}${progressRow(tx("send.weighted90"), totals.pipeline, `${totals.target ? Math.round(totals.pipeline / totals.target * 100) : 0}% ${tx("send.additional")}`, "#3b82f6")}<table role="presentation" style="width:100%;border-collapse:separate;border-spacing:0;background:#f8fafc;border:1px solid #dbe2ea;border-radius:6px;font-size:14px"><tr><td style="padding:14px 16px 8px;font-weight:bold;text-align:left">${tx("send.expectedTotal")}: ${money(expected)} / ${money(totals.target)} (${expectedPercent}% ${tx("send.coverage")})</td><td style="padding:14px 16px 8px;font-weight:bold;text-align:right;white-space:nowrap">${tx("send.remainingGap")}: ${money(remaining)}</td></tr><tr><td colspan="2" style="padding:0 16px 14px"><table role="presentation" style="width:100%;height:12px;border-collapse:separate;border-spacing:0;background:#e5e7eb;border-radius:99px;overflow:hidden"><tr><td style="width:${totals.target ? Math.min(100, totals.won / totals.target * 100) : 0}%;height:12px;padding:0;background:#10b981"></td><td style="width:${totals.target ? Math.min(100, Math.max(0, totals.pipeline / totals.target * 100)) : 0}%;height:12px;padding:0;background:#3b82f6"></td><td style="height:12px;padding:0;background:#e5e7eb"></td></tr></table></td></tr></table></section><section style="padding:26px 0;border-bottom:1px solid #dbe2ea"><h2 style="font-size:13px;text-transform:uppercase;letter-spacing:.08em;color:#667085">2. ${tx("send.partners")}</h2><table style="width:100%;table-layout:fixed;border-collapse:collapse;font-size:13px"><colgroup><col style="width:28%" /><col style="width:18%" /><col style="width:18%" /><col style="width:18%" /><col style="width:18%" /></colgroup><thead><tr><th style="padding:8px 6px;text-align:left">${tx("send.partner")}</th><th style="padding:8px 6px;text-align:left">${tx("send.country")}</th><th style="padding:8px 6px;text-align:center">Won</th><th style="padding:8px 6px;text-align:center">Q3/Q4 Pipeline</th><th style="padding:8px 6px;text-align:center">Forecast</th></tr></thead><tbody>${partnerRows}</tbody></table></section><section style="padding:26px 0;border-bottom:1px solid #dbe2ea"><h2 style="font-size:13px;text-transform:uppercase;letter-spacing:.08em;color:#667085">3. ${tx("send.hot")} (Next 90 Days)</h2><table style="width:100%;table-layout:fixed;border-collapse:collapse;font-size:13px"><colgroup><col style="width:24%" /><col style="width:18%" /><col style="width:18%" /><col style="width:14%" /><col style="width:12%" /><col style="width:14%" /></colgroup><thead><tr><th style="padding:8px 6px;text-align:left">${tx("send.clientDeal")}</th><th style="padding:8px 6px;text-align:left">${tx("send.partner")}</th><th style="padding:8px 6px;text-align:left">Client</th><th style="padding:8px 6px;text-align:center">${tx("send.amount")}</th><th style="padding:8px 6px;text-align:center">${tx("send.probability")}</th><th style="padding:8px 6px;text-align:center">${tx("send.estimatedClose")}</th></tr></thead><tbody>${opportunityRows}</tbody></table></section><section style="padding:26px 0;border-bottom:1px solid #dbe2ea"><h2 style="font-size:13px;text-transform:uppercase;letter-spacing:.08em;color:#667085">4. ${tx("send.criticalActions")}</h2><div style="background:#fff8eb;border:1px solid #f2d675;border-radius:6px;padding:14px">${impactRows}</div></section><section style="padding:26px 0"><h2 style="font-size:13px;text-transform:uppercase;letter-spacing:.08em;color:#667085">5. ${tx("send.expansion")}</h2><table style="width:100%;table-layout:fixed;border-collapse:collapse;font-size:13px"><colgroup><col style="width:34%" /><col style="width:22%" /><col style="width:18%" /><col style="width:26%" /></colgroup><thead><tr><th style="padding:8px 6px;text-align:left">${tx("send.partner")}</th><th style="padding:8px 6px;text-align:left">${tx("send.country")}</th><th style="padding:8px 18px;text-align:center">${tx("send.probability")}</th><th style="padding:8px 6px;text-align:center">${tx("send.onboardingDate")}</th></tr></thead><tbody>${expansionRows || `<tr><td colspan="4">${tx("send.noDate")}</td></tr>`}</tbody></table></section></article></div>`
    try {
      const translationPairs = Object.values(STATUS_MODAL_TRANSLATIONS).flatMap((entry) => [[entry.es, entry.en, "en"], [entry.es, entry.pt, "pt"]] as const)
      const localizeHtml = (locale: string) => {
        const normalized = locale.toLowerCase().slice(0, 2)
        if (normalized === "es") return html
        return translationPairs.filter(([source, target, language]) => language === normalized && source !== target).sort((a, b) => b[0].length - a[0].length).reduce((content, [source, target]) => content.split(source).join(target), html)
      }
      const selectedRecipientRecords = recipients.filter((recipient) => emailRecipients.includes(recipient.email))
      const messages = emailRecipients.map((email) => {
        const recipient = selectedRecipientRecords.find((item) => item.email === email)
        const language = recipient?.preferred_language || "en"
        return { to: email, subject: language.toLowerCase().startsWith("en") ? `Weekly Status - ${techCompanyName} - ${year}` : language.toLowerCase().startsWith("pt") ? `Status semanal - ${techCompanyName} - ${year}` : `Status semanal - ${techCompanyName} - ${year}`, html: localizeHtml(language) }
      })
      const response = await fetch("/api/status-techcompany/send", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages, recipients: emailRecipients, subject: `Weekly Status - ${techCompanyName} - ${year}`, html, techCompanyId, year, idempotencyKey: `techcompany-status/${techCompanyId}/${year}/${Date.now()}` }) })
