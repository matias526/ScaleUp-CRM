...
export function SendStatusModal({ open, onOpenChange, techCompanyName, techCompanyId, year, partners, inProcessPartners, onCancel }: SendStatusModalProps) {
  ...
  const sendEmail = async () => {
    ...
    const partnerRows = partners.map((partner) => `<tr><td>${partner.name}</td><td>${partner.countryName ?? "—"}</td><td style="text-align:center">${money(partner.won)}</td><td style="text-align:center">${money(partner.pipeline)}</td><td style="text-align:center">${money(partner.target)}</td></tr>`).join("")
    const opportunityRows = opportunities.map((opportunity) => `<tr><td>${opportunity.title}</td><td>${opportunity.partner}</td><td>${opportunity.customerName}</td><td style="text-align:center">${money(opportunity.amount)}</td><td style="text-align:center">${opportunity.probability}%</td><td style="text-align:center">${date(opportunity.closeDate, "en")}</td></tr>`).join("")
