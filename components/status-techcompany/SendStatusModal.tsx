...
export function SendStatusModal({ open, onOpenChange, techCompanyName, techCompanyId, year, partners, inProcessPartners, onCancel }: SendStatusModalProps) {
  ...
  const downloadPdf = () => {
    ...
    let y = 48
    const addText = (text: string, size = 10, bold = false) => { pdf.setFont("helvetica", bold ? "bold" : "normal"); pdf.setFontSize(size); const lines = pdf.splitTextToSize(text, pageWidth - margin * 2); pdf.text(lines, margin, y); y += lines.length * (size + 4) + 6 }
    const divider = () => { pdf.setDrawColor(220, 226, 234); pdf.line(margin, y, pageWidth - margin, y); y += 18 }
