...
export function InternalForecastAdjustment({ open, onOpenChange, projections, year = "2026", partnerName = "Partner", techCompanyName = "TechCompany", onSave }: Props) {
  ...
  const rows = useMemo(() => quarters.map((quarter) => draft.find((item) => item.period_quarter === quarter)).filter(Boolean) as QuarterlyProjection[], [draft])
  ...
  const updateQuarter = (quarter: QuarterlyProjection["period_quarter"], value: string) => setDraft((current) => current.map((row) => row.period_quarter === quarter ? { ...row, scaleup_internal_target_revenue: Number(value.replace(/[^0-9.-]/g, "")) || 0, display_target_type: displayTarget } : row))
